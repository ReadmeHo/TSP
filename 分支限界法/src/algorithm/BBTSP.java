...
public class BBTSP{ 
    ...
    public static float [][]distance;   //图G的临界矩阵
    ...
    public static boolean istrue(float[][]a, int[]x,HeapNode enode,float bestc){
	boolean result;
	...
	while(enode!=null && enode.s<n-1){//非叶结点
            x=enode.x;
	    //当前扩展结点是叶结点的父结点
            if(enode.s==n-2){
                //再加2条边构成回路,若所构成回路优于当前最优解，则找到费用更小的回路
                if(istrue(distance,x,enode,bestc))
                    shorterway();
	    }
	    //当前扩展结点不是叶结点的父结点
            else
